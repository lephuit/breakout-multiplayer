...
class Controller_Score extends Controller_Rest {
    ...
    public function action_create() {
            ...
            if (\Auth::check()) {
                ...
                $produit = Model_Score::forge(array(
                            'niveau' => Input::get('niveau'),
                            'briques' => Input::get('briques'),
                            'user_id' => $user_id,
                            'temps' => Input::get('temps'),
                        ));

                if ($produit and $produit->save()) {
                    return $this->response('Votre score as été enregistré !');
                } else {
                    return $this->response('Oups, il y à eu une erreur, votre score n\'as pas pu être enregistré');
                }
            }
            return $this->response(Input::get('temps'));
    }
